# documents/botdoc/consent/flow.py
import json
...
from documents.botdoc.consent.logic import prepare_bot_consent_data
...
class BotConsentForm(StatesGroup):
    answering = State()


BOT_CONSENT_INTRO_TEXT = (
    "📄 <b>Согласие на обработку персональных данных для бота</b>\n\n"
    "Отдельный документ для галочки «Согласен», который пользователь ставит при отправке данных в боте.\n\n"
    "🔒 <b>Безопасность:</b> вы вводите данные только для генерации. После создания файла вся информация удаляется автоматически."
)

BOT_CONSENT_WARNING_TEXT = (
...
def get_bot_consent_intro_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="▶️ Заполнить", callback_data="botconsent:fill")],
        [InlineKeyboardButton(text="🔙 Назад в меню", callback_data="menu:back")],
    ])


def get_bot_consent_warning_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="▶️ Начнём", callback_data="botconsent:start_questions")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="botdoc:intro")],
    ])


def get_bot_consent_complete_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄 Заполнить ещё раз", callback_data="botconsent:fill")],
...
async def show_bot_consent_intro(callback: types.CallbackQuery, check_sub: bool = True):
    ...
    await callback.message.edit_text(
        text=BOT_CONSENT_INTRO_TEXT,
        reply_markup=get_bot_consent_intro_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()


async def show_bot_consent_warning(callback: types.CallbackQuery):
    questions_count = load_questions_count()
    text = BOT_CONSENT_WARNING_TEXT.format(questions_count=questions_count)

    await callback.message.edit_text(
        text=text,
        reply_markup=get_bot_consent_warning_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def on_questionnaire_complete(callback: types.CallbackQuery, fsm: FSMContext):
    ...
    prepared = prepare_bot_consent_data(answers)

    template_path = Path(__file__).parent / "template.html"
    gen = DocumentGenerator()
    html_content = gen.render_template(str(template_path), prepared)

    pdf_file = generate_pdf(html_content, "bot_consent.pdf")
    word_file = generate_word(html_content, "bot_consent.docx")

    await callback.message.answer(BOT_CONSENT_COMPLETE_TEXT, parse_mode="HTML")
    ...
    user_id = callback.from_user.id
    log_generation(user_id, "botdoc_consent")

    await fsm.clear()

    await callback.message.answer(
        "Что делаем дальше?",
        reply_markup=get_bot_consent_complete_keyboard()
    )


@router.callback_query(F.data == "doc:bot_consent")
async def on_bot_consent_selected(callback: types.CallbackQuery):
    await show_bot_consent_intro(callback, check_sub=True)


@router.callback_query(F.data == "botdoc:intro")
async def on_botdoc_intro_back(callback: types.CallbackQuery, fsm: FSMContext):
    await fsm.clear()
    await show_bot_consent_intro(callback, check_sub=False)


@router.callback_query(F.data == "botconsent:fill")
async def on_botconsent_fill_selected(callback: types.CallbackQuery):
    await show_bot_consent_warning(callback)


@router.callback_query(F.data == "botconsent:start_questions")
async def on_botconsent_start_questions(callback: types.CallbackQuery, fsm: FSMContext):
    await fsm.set_state(BotConsentForm.answering)
    await fsm.update_data(answers={}, current_question_index=0)
    await show_question(callback, fsm, 0)


@router.callback_query(BotConsentForm.answering, F.data.startswith("ans:"))
...
@router.callback_query(F.data == "botconsent:complete")
async def on_botconsent_complete(callback: types.CallbackQuery):
    await callback.message.edit_text(
        text=BOT_CONSENT_COMPLETE_TEXT,
        reply_markup=get_bot_consent_complete_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()
...
@router.message(BotConsentForm.answering)
