# documents/botdoc/newsletter/flow.py
import json
...
from documents.botdoc.newsletter.logic import prepare_bot_newsletter_data
...
class BotNewsletterForm(StatesGroup):
    answering = State()


BOT_NEWSLETTER_INTRO_TEXT = (
    "📬 <b>Согласие на рассылку для Telegram-бота</b>\n\n"
    "Отдельный документ для согласия на рассылки в боте. Требуется по закону о рекламе.\n\n"
    "🔒 <b>Безопасность:</b> вы вводите данные только для генерации. После создания файла вся информация удаляется автоматически."
)

BOT_NEWSLETTER_WARNING_TEXT = (
...
def get_bot_newsletter_intro_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="▶️ Заполнить", callback_data="botnewsletter:fill")],
        [InlineKeyboardButton(text="🔙 Назад в меню", callback_data="menu:back")],
    ])


def get_bot_newsletter_warning_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="▶️ Начнём", callback_data="botnewsletter:start_questions")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="botdoc:intro")],
    ])


def get_bot_newsletter_complete_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄 Заполнить ещё раз", callback_data="botnewsletter:fill")],
...
async def show_bot_newsletter_intro(callback: types.CallbackQuery, check_sub: bool = True):
    ...
    await callback.message.edit_text(
        text=BOT_NEWSLETTER_INTRO_TEXT,
        reply_markup=get_bot_newsletter_intro_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()


async def show_bot_newsletter_warning(callback: types.CallbackQuery):
    questions_count = load_questions_count()
    text = BOT_NEWSLETTER_WARNING_TEXT.format(questions_count=questions_count)

    await callback.message.edit_text(
        text=text,
        reply_markup=get_bot_newsletter_warning_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def on_questionnaire_complete(callback: types.CallbackQuery, fsm: FSMContext):
    ...
    prepared = prepare_bot_newsletter_data(answers)

    template_path = Path(__file__).parent / "template.html"
    gen = DocumentGenerator()
    html_content = gen.render_template(str(template_path), prepared)

    pdf_file = generate_pdf(html_content, "bot_newsletter_consent.pdf")
    word_file = generate_word(html_content, "bot_newsletter_consent.docx")

    await callback.message.answer(BOT_NEWSLETTER_COMPLETE_TEXT, parse_mode="HTML")
    ...
    user_id = callback.from_user.id
    log_generation(user_id, "botdoc_newsletter")

    await fsm.clear()

    await callback.message.answer(
        "Что делаем дальше?",
        reply_markup=get_bot_newsletter_complete_keyboard()
    )


@router.callback_query(F.data == "doc:bot_newsletter")
async def on_bot_newsletter_selected(callback: types.CallbackQuery):
    await show_bot_newsletter_intro(callback, check_sub=True)


@router.callback_query(F.data == "botdoc:intro")
async def on_botdoc_intro_back(callback: types.CallbackQuery, fsm: FSMContext):
    await fsm.clear()
    await show_bot_newsletter_intro(callback, check_sub=False)


@router.callback_query(F.data == "botnewsletter:fill")
async def on_botnewsletter_fill_selected(callback: types.CallbackQuery):
    await show_bot_newsletter_warning(callback)


@router.callback_query(F.data == "botnewsletter:start_questions")
async def on_botnewsletter_start_questions(callback: types.CallbackQuery, fsm: FSMContext):
    await fsm.set_state(BotNewsletterForm.answering)
    await fsm.update_data(answers={}, current_question_index=0)
    await show_question(callback, fsm, 0)


@router.callback_query(BotNewsletterForm.answering, F.data.startswith("ans:"))
...
@router.callback_query(F.data == "botnewsletter:complete")
async def on_botnewsletter_complete(callback: types.CallbackQuery):
    await callback.message.edit_text(
        text=BOT_NEWSLETTER_COMPLETE_TEXT,
        reply_markup=get_bot_newsletter_complete_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()
...
@router.message(BotNewsletterForm.answering)
