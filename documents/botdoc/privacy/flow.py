# documents/botdoc/privacy/flow.py
import json
...
from documents.botdoc.privacy.logic import prepare_bot_privacy_data
...
class BotPrivacyForm(StatesGroup):
    answering = State()


BOT_PRIVACY_INTRO_TEXT = (
    "📄 <b>Политика конфиденциальности для Telegram-бота</b>\n\n"
    "Обязательный документ для бота, который собирает данные пользователей.\n\n"
    "🔒 <b>Безопасность:</b> вы вводите данные только для генерации. После создания файла вся информация удаляется автоматически."
)

BOT_PRIVACY_WARNING_TEXT = (
...
def get_bot_privacy_intro_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="▶️ Заполнить", callback_data="botprivacy:fill")],
        [InlineKeyboardButton(text="🔙 Назад в меню", callback_data="menu:back")],
    ])


def get_bot_privacy_warning_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="▶️ Начнём", callback_data="botprivacy:start_questions")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="botdoc:intro")],
    ])


def get_bot_privacy_complete_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄 Заполнить ещё раз", callback_data="botprivacy:fill")],
...
async def show_bot_privacy_intro(callback: types.CallbackQuery, check_sub: bool = True):
    ...
    await callback.message.edit_text(
        text=BOT_PRIVACY_INTRO_TEXT,
        reply_markup=get_bot_privacy_intro_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()


async def show_bot_privacy_warning(callback: types.CallbackQuery):
    questions_count = load_questions_count()
    text = BOT_PRIVACY_WARNING_TEXT.format(questions_count=questions_count)

    await callback.message.edit_text(
        text=text,
        reply_markup=get_bot_privacy_warning_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def on_questionnaire_complete(callback: types.CallbackQuery, fsm: FSMContext):
    ...
    prepared = prepare_bot_privacy_data(answers)

    template_path = Path(__file__).parent / "template.html"
    gen = DocumentGenerator()
    html_content = gen.render_template(str(template_path), prepared)

    pdf_file = generate_pdf(html_content, "bot_privacy_policy.pdf")
    word_file = generate_word(html_content, "bot_privacy_policy.docx")

    await callback.message.answer(BOT_PRIVACY_COMPLETE_TEXT, parse_mode="HTML")
    ...
    user_id = callback.from_user.id
    log_generation(user_id, "botdoc_privacy")

    await fsm.clear()

    await callback.message.answer(
        "Что делаем дальше?",
        reply_markup=get_bot_privacy_complete_keyboard()
    )


@router.callback_query(F.data == "category:bot")
async def on_bot_selected(callback: types.CallbackQuery):
    await show_bot_privacy_intro(callback, check_sub=True)


@router.callback_query(F.data == "botdoc:intro")
async def on_botdoc_intro_back(callback: types.CallbackQuery, fsm: FSMContext):
    await fsm.clear()
    await show_bot_privacy_intro(callback, check_sub=False)


@router.callback_query(F.data == "botprivacy:fill")
async def on_botprivacy_fill_selected(callback: types.CallbackQuery):
    await show_bot_privacy_warning(callback)


@router.callback_query(F.data == "botprivacy:start_questions")
async def on_botprivacy_start_questions(callback: types.CallbackQuery, fsm: FSMContext):
    await fsm.set_state(BotPrivacyForm.answering)
    await fsm.update_data(answers={}, current_question_index=0)
    await show_question(callback, fsm, 0)


@router.callback_query(BotPrivacyForm.answering, F.data.startswith("ans:"))
...
@router.callback_query(F.data == "botprivacy:complete")
async def on_botprivacy_complete(callback: types.CallbackQuery):
    await callback.message.edit_text(
        text=BOT_PRIVACY_COMPLETE_TEXT,
        reply_markup=get_bot_privacy_complete_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()
...
@router.message(BotPrivacyForm.answering)
