# documents/website/newsletter/flow.py
import json
...
from documents.website.newsletter.logic import prepare_newsletter_data
...
class NewsletterForm(StatesGroup):
    answering = State()


NEWSLETTER_INTRO_TEXT = (
    "📬 <b>Согласие на рассылку для сайта</b>\n\n"
    "Отдельный документ для согласия на email/sms/push-рассылки. Требуется по закону о рекламе.\n\n"
    "🔒 <b>Безопасность:</b> вы вводите данные только для генерации. После создания файла вся информация удаляется автоматически."
)

NEWSLETTER_WARNING_TEXT = (
...
def get_newsletter_intro_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="▶️ Заполнить", callback_data="newsletter:fill")],
        [InlineKeyboardButton(text="🔙 Назад в меню", callback_data="menu:back")],
    ])


def get_newsletter_warning_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="▶️ Начнём", callback_data="newsletter:start_questions")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="website:intro")],
    ])


def get_newsletter_complete_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄 Заполнить ещё раз", callback_data="newsletter:fill")],
...
async def show_newsletter_intro(callback: types.CallbackQuery, check_sub: bool = True):
    ...
    await callback.message.edit_text(
        text=NEWSLETTER_INTRO_TEXT,
        reply_markup=get_newsletter_intro_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()


async def show_newsletter_warning(callback: types.CallbackQuery):
    questions_count = load_questions_count()
    text = NEWSLETTER_WARNING_TEXT.format(questions_count=questions_count)

    await callback.message.edit_text(
        text=text,
        reply_markup=get_newsletter_warning_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def on_questionnaire_complete(callback: types.CallbackQuery, fsm: FSMContext):
    ...
    prepared = prepare_newsletter_data(answers)

    template_path = Path(__file__).parent / "template.html"
    gen = DocumentGenerator()
    html_content = gen.render_template(str(template_path), prepared)

    pdf_file = generate_pdf(html_content, "newsletter_consent.pdf")
    word_file = generate_word(html_content, "newsletter_consent.docx")

    await callback.message.answer(NEWSLETTER_COMPLETE_TEXT, parse_mode="HTML")
    ...
    user_id = callback.from_user.id
    log_generation(user_id, "website_newsletter")

    await fsm.clear()

    await callback.message.answer(
        "Что делаем дальше?",
        reply_markup=get_newsletter_complete_keyboard()
    )


@router.callback_query(F.data == "doc:website_newsletter")
async def on_newsletter_selected(callback: types.CallbackQuery):
    await show_newsletter_intro(callback, check_sub=True)


@router.callback_query(F.data == "website:intro")
async def on_website_intro_back(callback: types.CallbackQuery, fsm: FSMContext):
    await fsm.clear()
    await show_newsletter_intro(callback, check_sub=False)


@router.callback_query(F.data == "newsletter:fill")
async def on_newsletter_fill_selected(callback: types.CallbackQuery):
    await show_newsletter_warning(callback)


@router.callback_query(F.data == "newsletter:start_questions")
async def on_newsletter_start_questions(callback: types.CallbackQuery, fsm: FSMContext):
    await fsm.set_state(NewsletterForm.answering)
    await fsm.update_data(answers={}, current_question_index=0)
    await show_question(callback, fsm, 0)


@router.callback_query(NewsletterForm.answering, F.data.startswith("ans:"))
...
@router.callback_query(F.data == "newsletter:complete")
async def on_newsletter_complete(callback: types.CallbackQuery):
    await callback.message.edit_text(
        text=NEWSLETTER_COMPLETE_TEXT,
        reply_markup=get_newsletter_complete_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()
...
@router.message(NewsletterForm.answering)
