# documents/website/advertising/flow.py
import json
...
from documents.website.advertising.logic import prepare_advertising_data
...
class AdvertisingForm(StatesGroup):
    answering = State()


ADVERTISING_INTRO_TEXT = (
    "📢 <b>Согласие на рекламу для сайта</b>\n\n"
    "Документ для cookie, таргетированной рекламы, Яндекс.Метрики и других маркетинговых инструментов.\n\n"
    "🔒 <b>Безопасность:</b> вы вводите данные только для генерации. После создания файла вся информация удаляется автоматически."
)

ADVERTISING_WARNING_TEXT = (
...
def get_advertising_intro_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="▶️ Заполнить", callback_data="advertising:fill")],
        [InlineKeyboardButton(text="🔙 Назад в меню", callback_data="menu:back")],
    ])


def get_advertising_warning_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="▶️ Начнём", callback_data="advertising:start_questions")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="website:intro")],
    ])


def get_advertising_complete_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄 Заполнить ещё раз", callback_data="advertising:fill")],
...
async def show_advertising_intro(callback: types.CallbackQuery, check_sub: bool = True):
    ...
    await callback.message.edit_text(
        text=ADVERTISING_INTRO_TEXT,
        reply_markup=get_advertising_intro_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()


async def show_advertising_warning(callback: types.CallbackQuery):
    questions_count = load_questions_count()
    text = ADVERTISING_WARNING_TEXT.format(questions_count=questions_count)

    await callback.message.edit_text(
        text=text,
        reply_markup=get_advertising_warning_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def on_questionnaire_complete(callback: types.CallbackQuery, fsm: FSMContext):
    ...
    prepared = prepare_advertising_data(answers)

    template_path = Path(__file__).parent / "template.html"
    gen = DocumentGenerator()
    html_content = gen.render_template(str(template_path), prepared)

    pdf_file = generate_pdf(html_content, "advertising_consent.pdf")
    word_file = generate_word(html_content, "advertising_consent.docx")

    await callback.message.answer(ADVERTISING_COMPLETE_TEXT, parse_mode="HTML")
    ...
    user_id = callback.from_user.id
    log_generation(user_id, "website_advertising")

    await fsm.clear()

    await callback.message.answer(
        "Что делаем дальше?",
        reply_markup=get_advertising_complete_keyboard()
    )


@router.callback_query(F.data == "doc:website_advertising")
async def on_advertising_selected(callback: types.CallbackQuery):
    await show_advertising_intro(callback, check_sub=True)


@router.callback_query(F.data == "website:intro")
async def on_website_intro_back(callback: types.CallbackQuery, fsm: FSMContext):
    await fsm.clear()
    await show_advertising_intro(callback, check_sub=False)


@router.callback_query(F.data == "advertising:fill")
async def on_advertising_fill_selected(callback: types.CallbackQuery):
    await show_advertising_warning(callback)


@router.callback_query(F.data == "advertising:start_questions")
async def on_advertising_start_questions(callback: types.CallbackQuery, fsm: FSMContext):
    await fsm.set_state(AdvertisingForm.answering)
    await fsm.update_data(answers={}, current_question_index=0)
    await show_question(callback, fsm, 0)


@router.callback_query(AdvertisingForm.answering, F.data.startswith("ans:"))
...
@router.callback_query(F.data == "advertising:complete")
async def on_advertising_complete(callback: types.CallbackQuery):
    await callback.message.edit_text(
        text=ADVERTISING_COMPLETE_TEXT,
        reply_markup=get_advertising_complete_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()
...
@router.message(AdvertisingForm.answering)
