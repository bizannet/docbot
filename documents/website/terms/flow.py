# documents/website/terms/flow.py
import json
...
from documents.website.terms.logic import prepare_terms_data
...
class TermsForm(StatesGroup):
    answering = State()


TERMS_INTRO_TEXT = (
    "📄 <b>Пользовательское соглашение для сайта</b>\n\n"
    "Правила использования сайта для пользователей: что можно, что нельзя, ответственность.\n\n"
...
def get_terms_intro_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="▶️ Заполнить", callback_data="terms:fill")],
        [InlineKeyboardButton(text="🔙 Назад в меню", callback_data="menu:back")],
    ])


def get_terms_warning_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="▶️ Начнём", callback_data="terms:start_questions")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="website:intro")],
    ])


def get_terms_complete_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄 Заполнить ещё раз", callback_data="terms:fill")],
...
async def show_terms_intro(callback: types.CallbackQuery, check_sub: bool = True):
    ...
    await callback.message.edit_text(
        text=TERMS_INTRO_TEXT,
        reply_markup=get_terms_intro_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()


async def show_terms_warning(callback: types.CallbackQuery):
    questions_count = load_questions_count()
    text = TERMS_WARNING_TEXT.format(questions_count=questions_count)

    await callback.message.edit_text(
        text=text,
        reply_markup=get_terms_warning_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def on_questionnaire_complete(callback: types.CallbackQuery, fsm: FSMContext):
    ...
    prepared = prepare_terms_data(answers)

    template_path = Path(__file__).parent / "template.html"
    gen = DocumentGenerator()
    html_content = gen.render_template(str(template_path), prepared)

    pdf_file = generate_pdf(html_content, "terms_of_use.pdf")
    word_file = generate_word(html_content, "terms_of_use.docx")

    await callback.message.answer(TERMS_COMPLETE_TEXT, parse_mode="HTML")
    ...
    user_id = callback.from_user.id
    log_generation(user_id, "website_terms")

    await fsm.clear()

    await callback.message.answer(
        "Что делаем дальше?",
        reply_markup=get_terms_complete_keyboard()
    )


@router.callback_query(F.data == "doc:website_terms")
async def on_terms_selected(callback: types.CallbackQuery):
    await show_terms_intro(callback, check_sub=True)


@router.callback_query(F.data == "website:intro")
async def on_website_intro_back(callback: types.CallbackQuery, fsm: FSMContext):
    await fsm.clear()
    await show_terms_intro(callback, check_sub=False)


@router.callback_query(F.data == "terms:fill")
async def on_terms_fill_selected(callback: types.CallbackQuery):
    await show_terms_warning(callback)


@router.callback_query(F.data == "terms:start_questions")
async def on_terms_start_questions(callback: types.CallbackQuery, fsm: FSMContext):
    await fsm.set_state(TermsForm.answering)
    await fsm.update_data(answers={}, current_question_index=0)
    await show_question(callback, fsm, 0)


@router.callback_query(TermsForm.answering, F.data.startswith("ans:"))
...
@router.callback_query(F.data == "terms:complete")
async def on_terms_complete(callback: types.CallbackQuery):
    await callback.message.edit_text(
        text=TERMS_COMPLETE_TEXT,
        reply_markup=get_terms_complete_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()
...
@router.message(TermsForm.answering)
