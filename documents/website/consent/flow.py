# documents/website/consent/flow.py
import json
...
from documents.website.consent.logic import prepare_consent_data
...
CONSENT_INTRO_TEXT = (
    ...
    "Отдельный документ для галочки «Согласен», который пользователь ставит при отправке данных.\n\n"
...
def get_consent_intro_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="▶️ Заполнить", callback_data="consent:fill")],
        [InlineKeyboardButton(text="🔙 Назад в меню", callback_data="menu:back")],
    ])


def get_consent_warning_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="▶️ Начнём", callback_data="consent:start_questions")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="website:intro")],
    ])


def get_consent_complete_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄 Заполнить ещё раз", callback_data="consent:fill")],
...
async def show_consent_intro(callback: types.CallbackQuery, check_sub: bool = True):
    ...
    await callback.message.edit_text(
        text=CONSENT_INTRO_TEXT,
        reply_markup=get_consent_intro_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()


async def show_consent_warning(callback: types.CallbackQuery):
    questions_count = load_questions_count()
    text = CONSENT_WARNING_TEXT.format(questions_count=questions_count)

    await callback.message.edit_text(
        text=text,
        reply_markup=get_consent_warning_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()
...
async def on_questionnaire_complete(callback: types.CallbackQuery, fsm: FSMContext):
    ...
    prepared = prepare_consent_data(answers)

    template_path = Path(__file__).parent / "template.html"
    gen = DocumentGenerator()
    html_content = gen.render_template(str(template_path), prepared)

    pdf_file = generate_pdf(html_content, "consent.pdf")
    word_file = generate_word(html_content, "consent.docx")

    await callback.message.answer(CONSENT_COMPLETE_TEXT, parse_mode="HTML")
    ...
    user_id = callback.from_user.id
    log_generation(user_id, "website_consent")

    await fsm.clear()

    await callback.message.answer(
        "Что делаем дальше?",
        reply_markup=get_consent_complete_keyboard()
    )


@router.callback_query(F.data == "doc:website_consent")
async def on_consent_selected(callback: types.CallbackQuery):
    await show_consent_intro(callback, check_sub=True)


@router.callback_query(F.data == "website:intro")
async def on_website_intro_back(callback: types.CallbackQuery, fsm: FSMContext):
    await fsm.clear()
    await show_consent_intro(callback, check_sub=False)


@router.callback_query(F.data == "consent:fill")
async def on_consent_fill_selected(callback: types.CallbackQuery):
    await show_consent_warning(callback)


@router.callback_query(F.data == "consent:start_questions")
async def on_consent_start_questions(callback: types.CallbackQuery, fsm: FSMContext):
    await fsm.set_state(ConsentForm.answering)
    await fsm.update_data(answers={}, current_question_index=0)
    await show_question(callback, fsm, 0)


@router.callback_query(ConsentForm.answering, F.data.startswith("ans:"))
...
@router.callback_query(F.data == "consent:complete")
async def on_consent_complete(callback: types.CallbackQuery):
    await callback.message.edit_text(
        text=CONSENT_COMPLETE_TEXT,
        reply_markup=get_consent_complete_keyboard(),
        parse_mode="HTML"
    )
    await callback.answer()
...
@router.message(ConsentForm.answering)
